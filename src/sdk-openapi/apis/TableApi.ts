...
export class TableApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getTableById(id: string, xContext?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'id' is not null or undefined
        if (id === null || id === undefined) {
            throw new RequiredError("TableApi", "getTableById", "id");
        }



        // Path Params
        const localVarPath = '/v1/tables/{id}'
     ...
     * @param page 
     */
    public async getTables(xContext?: string, size?: number, page?: number, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;
...
export class TableApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to getTableById
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getTableByIdWithHttpInfo(response: ResponseContext): Promise<HttpInfo<TableDto >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TableDto = ObjectSerializer.deserialize(
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getTables
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getTablesWithHttpInfo(response: ResponseContext): Promise<HttpInfo<GetTables200Response >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: GetTables200Response = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "GetTables200Response", ""
            ) as GetTables200Response;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
        if (isCodeInRange("400", response.httpStatusCode)) {
            const body: GetTaskById400Response = ObjectSerializer.deserialize(
